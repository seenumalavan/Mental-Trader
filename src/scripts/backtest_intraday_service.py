"""Backtest the existing IntradayService end-to-end by replaying historical candles as synthetic ticks.

Non-invasive: does not modify service/strategy/executor code. We:
  1. Resolve instruments.
  2. Fetch historical candles (via Upstox HistoryV3Api) for a date range.
  3. Initialize IntradayService (warmup uses loaded candles + intraday fetch skipped via flag).
...
Usage (Windows CMD):
  python -m src.scripts.backtest_intraday_service --symbols nifty,RELIANCE --start 2025-03-01 --end 2025-03-07 --limit 1200
...
# ---------------------- Replay logic ----------------------
async def replay_candles(service: IntradayService, symbol: str, instrument_key: str, candles: List[Dict[str, Any]]):
    ...
    """
    # We assume service.bar_builder will generate timeframe bars; drives intraday signals.
    for c in candles:
        tick = {
            'symbol': symbol,
...
def extract_positions(service: IntradayService) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
...
def parse_args():
    p = argparse.ArgumentParser(description="Backtest IntradayService by replaying historical candles")
